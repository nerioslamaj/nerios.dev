...
class Menu extends Component {
  ...
  handleScroll(event) {
    if (event.currentTarget.scrollY > this.state.lastScrollTop){
      this.setState({
        class: 'menu-up'
      });
    } else {
    ...
    this.setState({
      lastScrollTop: event.currentTarget.scrollY <= 0 ? 0 : event.currentTarget.scrollY
    });
  }
  ...
  handleMenuClick(element) {
    ...
    function offsetPositionTop(el) {
      return el&&el.nodeName!=='BODY'?el.offsetTop+offsetPositionTop(el.offsetParent):0;
    }
    window.scroll({ top: height, behavior: 'smooth' })
  }

  render() {
    var svgTag = `<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 198.999 29.999" enable-background="new 0 0 198.999 29.999" xml:space="preserve"><path fill-rule="evenodd" clip-rule="evenodd" d="M194.063,23.759c-2.137,0-3.316-1.291-3.733-1.802
    ...
    h-5.213v-5.14H8.566H3.938V8.375z"></path></svg>`
    return (
      <div className={"Menu row " + this.state.class} ref={this.menu}>
        <div className="container inner-menu">
          ...
          <ul>
            ...
            <li><h6 onClick={() => this.handleMenuClick('stack')}>Stack</h6></li>
            <li><h6 onClick={() => this.handleMenuClick('contact')}>Contact</h6></li>
          </ul>
        </div>
      </div>
    );
